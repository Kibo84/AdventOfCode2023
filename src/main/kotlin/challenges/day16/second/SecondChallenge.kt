package challenges.day16.second

import java.io.BufferedReader
import java.io.File
import java.io.InputStreamReader
import challenges.day16.second.Directions.*
...
fun calculateValue(): Int {
    ...
    val lastPositionX = matrix[firstPosition].size - indexAdjustment
    val lastPositionY = matrix.size - indexAdjustment

    matrix.forEachIndexed { lineIndex, line ->
        line.forEachIndexed { index, _ ->
            if (lineIndex == firstPosition) {
                resetCopyOfMatrix()
                calculateDirection(position = Position(index, lineIndex), from = UP)
                val tempResult = calculateValueOfCopy()

                if (tempResult > result) result = tempResult
            }
            if (lineIndex == lastPositionY) {
                resetCopyOfMatrix()
                calculateDirection(position = Position(index, lineIndex), from = DOWN)
                val tempResult = calculateValueOfCopy()
            ...
            if (index == firstPosition) {
                resetCopyOfMatrix()
                calculateDirection(position = Position(index, lineIndex), from = LEFT)
                val tempResult = calculateValueOfCopy()

                if (tempResult > result) result = tempResult
            }
            if (index == lastPositionX) {
                resetCopyOfMatrix()
                calculateDirection(position = Position(index, lineIndex), from = RIGHT)
                val tempResult = calculateValueOfCopy()
...
fun calculateNextPosition(position: Position, direction: Directions) {
    ...
    if (newPosition.isValidPosition()) {
        when (direction) {
            UP -> calculateDirection(newPosition, DOWN)
            LEFT -> calculateDirection(newPosition, RIGHT)
            RIGHT -> calculateDirection(newPosition, LEFT)
            DOWN -> calculateDirection(newPosition, UP)
        }
    }
}

fun resetCopyOfMatrix() {
    copyOfMatrix = matrix.map { line ->
        MutableList(line.size) { emptySpace }
    }
}

fun calculateValueOfCopy(): Int {
    return copyOfMatrix.sumOf { line -> line.count { it == energizedChar } }
}
